...
class _CharacterHomeScreenState extends State<CharacterHomeScreen> {
  ...
  // to prevent multiple requests (snack bar) when network connection is lost and the user is still scrolling to bottom to load more cards.
  ...
  void _onScroll() {
    ...
    // 1. لو الكيوبت قافل الريكويستات بسبب إيرور نت، اخرج واسكت تماماً
    if (cubit.isNetworkErrorLocked) return;

    // 2. الحماية أثناء التحميل الطبيعي
    if (currentState is CharacterPaginationLoadingState) {
      if (_scrollController.position.pixels > lockPosition) {
        return;
      }
    }

    // 3. حماية الكيوبت أثناء الشغل
    if (cubit.isPageLoading) return;

    // 4. الطلب التلقائي النظيف
    if (currentState is! CharacterPaginationLoadingState &&
  ...
  //   final double lockPosition = _scrollController.position.maxScrollExtent * 0.8;
  //   // at loading a new page, we lock the scroll at 80% of the max scroll extent to prevent multiple requests.
  //   if (currentState is CharacterPaginationLoadingState && _scrollController.position.pixels > lockPosition) {
  ...
  //appbar with user name, profile picture, search bar and filter button
      ...
      child: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            SizedBox(height: 2),
            Text(appUserName, style: TextStyle(color: AppColors.userNameColor,
                fontSize: 18,
                fontWeight: FontWeight.bold,
                letterSpacing: 0.2)),
          ],
  ...
  //bottom sheet to show filters options (not implemented yet, just a placeholder)
    ...
    showModalBottomSheet(
      ...
      builder: (context) =>
          Container(
            ...
            child: const Center(child: Text('Filter Bottom Sheet Placeholder',
                style: TextStyle(color: Colors.black45))),
          ),
    );
  }
}
// a separate widget for the characters grid to keep the code organized and readable.
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                if (isFirstPageLoading) {
                  return const CharacterCard(name: 'Loading Character Name',
                      status: 'Alive',
                      species: 'Human',
                      gender: 'Male');
                }
                ...
                if (index >= characters.length) {
                  return const Skeletonizer(
                    enabled: true,
                    child: CharacterCard(name: 'Loading More...',
                        status: 'Alive',
                        species: 'Alien',
                        gender: 'Female'),
                  );
